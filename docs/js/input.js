...
addEventListener('keydown',e=>{
  ...
  if(G.bindCapture){G.meta.binds[G.bindCapture]=k;G.bindCapture=null;saveMeta();if(window.Sound)Sound.ui();return;}
  ...
  if(G.state==='history'){if(k==='escape'||k==='backspace')G.state='setup';return;}
  ...
  if(actionPressed(k,'analytics') && G.state==='play'){ G.showAnalytics=!G.showAnalytics; if(G.showAnalytics){ G._surface=null; computeSurface(); } return; }
  ...
  if(actionPressed(k,'pause') && G.state==='play'){ G.state='paused';if(typeof saveRun==='function')saveRun(); }
  else if(actionPressed(k,'pause') && G.state==='paused') G.state='play';
...
cv.addEventListener('mousemove',e=>{ let p=cpos(e);
  if(G.meta.opts.view3d&&document.pointerLockElement===cv){G.fpAngle+=e.movementX*0.0025;G.pitch=Math.max(-0.75,Math.min(0.75,G.pitch-e.movementY*0.002));p={x:W/2,y:H/2};}
  mouse.x=p.x;mouse.y=p.y;dragSlider(p);if(setDragKey)setSlideVal(p);});
